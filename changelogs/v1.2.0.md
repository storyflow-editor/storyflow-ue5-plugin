...
description: "Adds full runtime support for the editor's map variable type including forEachMap loops, map-typed character variables, runScript map parameters and save game persistence. Also adds modulo and moduloFloat nodes, Blueprint array variable setters, a GetMapVariable accessor and a batch of evaluator fixes covering loops, arrays, float reads and enum conversions."
---
...
- `getCharacterVar` and `setCharacterVar` now support map-typed character variables; map chains sourced from character variables are read-only and are snapshotted when written via `setMap` so character data is never aliased
